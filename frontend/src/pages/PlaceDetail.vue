<template>
  <div class="detail-page">
    <div class="topbar">
      <router-link to="/places" class="back">← 返回地点库</router-link>
      <div class="site-title">
        数字记忆 · 知识详情 <span class="sub">dm.cqu.edu.cn</span>
      </div>
      <div class="actions">
    ...
    <div v-else-if="place" class="content">
      <h1 class="name">{{ place.name }}</h1>
      <div class="meta-row">
        <span>{{ place.readCount }} 阅读</span>
        <span>｜ 收藏</span>
      </div>
...
const fetchPlaceDetail = async (id) => {
  try {
    loading.value = true;
    console.log("加载地点详情，ID:", id);

    const response = await getEntityDetail(id);
    console.log("地点详情API返回:", response);
...
onMounted(() => {
  if (route.params.id) {
    fetchPlaceDetail(route.params.id);
  }
});
