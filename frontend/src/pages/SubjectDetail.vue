<template>
  <div class="detail-page">
    ...
    <div v-else-if="subject" class="content">
      <h1 class="name">{{ subject.name }}</h1>
      <div class="meta-row">
        <span>{{ subject.readCount }} 阅读</span>
        <span>｜ 收藏</span>
      </div>
      ...
      <div class="body">
        ...
        <div class="right">
          <div class="photo">
            <img :src="subject.photo" alt="photo" />
          </div>
        </div>
      </div>
...
const fetchSubjectDetail = async (id) => {
  try {
    loading.value = true;
    console.log("加载学科详情，ID:", id);

    const response = await getEntityDetail(id);
    console.log("学科详情API返回:", response);
...
onMounted(() => {
  if (route.params.id) {
    fetchSubjectDetail(route.params.id);
  }
});
